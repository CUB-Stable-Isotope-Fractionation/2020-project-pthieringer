...
The same paper by Etiope et al. (2011) has reported $\delta^{13}C_{CO_2}$ values from gas generated from one of the seeps. However, our study reported $\delta^{13}C_{lipid}$ to inference the residual DIC available to methanogens may be extremely enriched. Here we'll use a sequence of values to see what values may be representative and if they all fit into predicting energetic potentials. We're assuming the DIC values we are estimating are in the form of dissovled $CO_2$ for use by methanogens in our system.

```{r}
delta_CO2_reported <- seq(-0.018, -0.015, by = 0.001) #Values recorded from the head of a gas seep (Etiope et al. 2011)
delta_CO2_est <- seq(-0.014, 0.012, by = 0.001) #Predicting values higher than reported based off of the lipid-isotope values from Zwicker et al. (2018)
```
...
**What might those values represent? If we assume this relationship true, what would this plot suggest?**
